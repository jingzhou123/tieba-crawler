...
class PostPipeline(TbBasePipeline):
    ...
    def _do_upsert(self, conn, item, spider):
        ...
        conn.execute( """
            ...
            body=%s, post_time=%s, reply_num=%s, author_id=%s, id=%s
       """,
            (
                item['author_name'],
                item['tieba_name'],
...
class ReplyPipeline(TbBasePipeline):
    ...
    def _do_upsert(self, conn, item, spider):
        ...
        if item['type'] == 'MAIN':
            conn.execute("""
                UPDATE post SET title=%s, body=%s, post_time=%s, author_id=%s where id=%s
            """, (
                item['title'],
                item['body'],
                item['post_time'],
            ...
            conn.execute("""
                ...
                post_time=%s, post_id=%s, reply_num=%s, author_id=%s
            """, (
                item['author_name'],
                item['body'],
                item['id'],
